...
public class SlidingDrawer extends ViewGroup {
	...
	public SlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
		...
		int handleId = a
				.getResourceId(R.styleable.SlidingDrawer_panelHandle, 0);
		if (handleId == 0) {
		...
		int contentId = a.getResourceId(R.styleable.SlidingDrawer_panelContent,
				0);
		if (contentId == 0) {
			throw new IllegalArgumentException(
				...
				if (Math.abs(velocity) < mMaximumTapVelocity) {
					if (vertical ? (mExpanded && top < mTapThreshold
							...
							|| (!mExpanded && top > mBottomOffset + getBottom()
									- getTop() - mHandleHeight - mTapThreshold)
	...
	private void prepareContent() {
		...
		// content.buildDrawingCache();[jacky|fix interface conflict].
