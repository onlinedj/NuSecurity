...
public class SlidingDrawer extends ViewGroup {
    ...
    public SlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
        ...
        int handleId = a.getResourceId(R.styleable.SlidingDrawer_panelHandle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.SlidingDrawer_panelContent, 0);
        if (contentId == 0) {
    ...
    private void prepareContent() {
        ...
        //if (!content.isHardwareAccelerated()) content.buildDrawingCache(); [jacky|fix interface conflict].
